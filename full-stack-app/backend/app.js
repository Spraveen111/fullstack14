...
import whatsappSms from "./whatsapp.js";
...
app.post("/signinDetails", async (req, res) => {
  ...
  try {
    ...
    if (user.password === password) {
      const token = jwt.sign({ email: user.email }, "iloveyou", {
        expiresIn: "2h",
      });
      const messageBody = `Hi ${user.name}, You have Successfully Logged in Vegitables Website`;

      sendSMS(messageBody, user.phoneNumber);
      whatsappSms(messageBody, user.phoneNumber);
      res.status(200).send({ message: "User in the Database", token: token });
    } else {
  ...
  try {
    ...
    const insertResults = await Promise.all(
      dataArray.map(async (data) => {
        return await nonveg.insertOne(data);
      })
    );
...
app.get("/getAllVegetables", verifyToken, async (req, res) => {
